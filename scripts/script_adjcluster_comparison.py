...
parser.add_argument(
    ...
    nargs='+',
    type=str,
    default=["hd_1x2x6_lateralAPA", "vd_1x8x14_3view_30deg_nominal"],
...
parser.add_argument(
    '--labelx',
    type=str,
    default="Reconstructed Energy (MeV)",
...
def main():
    ...
    for idx, (variable, iterable) in enumerate(product(args.variables, args.iterables)):
        ...
        for (jdx, config), (kdx, value) in product(enumerate(args.configs), enumerate(sorted(unique_values))):
            ...
            label = "HD_lateralAPA" if config == "hd_1x2x6_lateralAPA" else "VD_nominal" if config == "vd_1x8x14_3view_30deg_nominal" else config
        ...
        plt.legend()
        plt.title(f"{variable} Distribution", fontsize=18)
        # dunestyle.WIP()
